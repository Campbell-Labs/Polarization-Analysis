            ...
            for i=1:length(dirList)
                ...
                if ~cancel
                    if createNew
                        ...
                        toFilename = eye.getFilename();
                        quarter = Quarter(suggestedQuarterNumber, eye.getQuarterNumbers(), toEyeProjectPath, projectPath, quarterImportPath, userName, toFilename);
                    else
        ...
        function eye = importLegacyData(eye, toEyeProjectPath, legacyImportPaths, displayImportPath, localProjectPath, dataFilename, userName, subjectType)
            ...
            if ~cancel
                if createNew
                    suggestedQuarterNumber = eye.nextQuarterNumber();
                    toFilename = eye.getFilename();
                    quarter = Quarter(suggestedQuarterNumber, eye.getQuarterNumbers(), toEyeProjectPath, localProjectPath, displayImportPath, userName, toFilename);
                else
                    quarter = eye.getQuarterFromChoice(choice);
                end
